...
// ═══════════════════════════════════════════════════════════════
// Thin wrapper around triggerCarrierVerification service.
// The core logic lives in app/src/services/verification.ts.

import { triggerCarrierVerification } from "../services/verification";

router.post("/api/verify/trigger", async (req, res) => {
  try {
    const result = await triggerCarrierVerification(req.body);

    // Notify broker if DO_NOT_USE (service doesn't send broker SMS — that's the caller's job)
    if (result.result === "DO_NOT_USE" && req.body.broker_phone) {
      await sendSms(req.body.broker_phone,
        `MC#${req.body.mc_number} — DO NOT USE. ${(result.reasons || []).join(". ")}. Details: ${result.verify_url}/report`
      );
    }
    ...
    const message = err instanceof Error ? err.message : "Verification trigger failed";
